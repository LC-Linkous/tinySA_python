...
def main():
    ...
    # write fixtures file ---------------------------------------------------
    # Provenance: pull the device identity out of the captured version/info
    # responses so the header records WHAT produced these bytes, not just when.
    def _first_line(label):
        rec = records.get(label) or {}
        raw = rec.get("cleaned") or rec.get("raw")
        if not raw:
            return "unknown"
        try:
    ...
    device_model = _first_line("info")       # e.g. 'tinySA ULTRA'
    device_firmware = _first_line("version") # e.g. 'tinySA4_v1.4-143-g864bb27'
