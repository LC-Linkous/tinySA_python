...
#
#   Complete end-to-end example: connect, read device info, collect a sweep with hop(),
#   and plot it. HARDWARE IS REQUIRED.
#
...
def run_complete_example():
    ...
    # matplotlib is an OPTIONAL dependency (plotting extra). Import it here, not
    # at module top, so the file stays importable without it installed.
    try:
        import matplotlib.pyplot as plt
    except ImportError:
        print("ERROR: this example needs matplotlib. "
              'Install the plotting extra:  pip install "tsapython[plotting]"')
        return False
    ...
    try:
        ...
        if connected_bool:
            ...
            print(f"Collecting data from {start_freq/1e6:.1f} MHz to "
                  f"{stop_freq/1e6:.1f} MHz ({n_pts} points)...")
            ...
            print(f"PASS! Frequency data collected: "
                  f"{len(freq_vals.decode('utf-8').split())} points")
            ...
            print(f"PASS! Power data collected: "
                  f"{len(dbm_vals.decode('utf-8').split())} points")
            ...
            try:
                ...
                plt.title("tinySA Hop() Plot - Complete Example",
